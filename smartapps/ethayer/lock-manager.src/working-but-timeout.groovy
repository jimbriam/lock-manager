...
def initializeMain() {
	...
	log.debug("=====initializeMain")
	log.debug("${codes} ${requestCount} ${sweepMode}")
...
def theAppType() {
	log.debug("theAppType ${state.toString().length()}")
  if (parent) {
	log.debug("theAppType: parent")
...
def collectCodesToUnset() {
  def codes = state.codes
	log.debug("collectCodesToUnset ${codes.size()}" )
  def incorrectSlots = []
  def array = []
  def count = 0
...
def collectCodesToSet() {
	log.debug("collectCodesToSet ${state.codes}")
  def codes = state.codes
...
def findSlotUserApp(slot) {
	log.debug("findSlotUserApp ${slot}")
  if (slot) {
    def lockUsers = parent.getUserApps()
...
/* Calculate the usage of a lock for a user 
	*/
def totalUsage() {
  def usage = 0
